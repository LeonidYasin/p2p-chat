export const GO_CODE_MAIN = `package main
...
export const GO_CODE_MOD = `module p2pchat
...
export const GO_CODE_README = `# Go libp2p Rendezvous Chat Messenger

A production-ready terminal core demonstrating the **libp2p** framework in Go. It connects peers automatically using two distinct discovery mechanisms:
1. **Multicast DNS (mDNS)**: Automatically detects and connects peers running on the *same local network (LAN)* without any configurations.
2. **Kademlia DHT Rendezvous**: Registers nodes in a decentralized routing table under a specific room key (e.g., \`chat-with-rendezvous\`). Peers on the *wider internet or separate address spaces* find each other via common DHT bootstrap anchors.

---

## 🛠 Prerequisites

1. Install **Go 1.20** or higher on your system:
...
Navigate to the directory housing \`main.go\` and \`go.mod\`, then fetch external libp2p modules:
\`\`\`bash
go mod tidy
\`\`\`
...
Open multiple terminal terminals on your machine (or different machines on your subnet) to see real-time peer-discovery!

**Start Node A:**
\`\`\`bash
./p2pchat -port 3001 -nick Alice -room alpha-lobby
\`\`\`

**Start Node B (in a different terminal):**
\`\`\`bash
./p2pchat -port 3002 -nick Bob -room alpha-lobby
\`\`\`
...
Once running, type your text directly to broadcast to connected peers, or use these standard commands:
- \`/peers\` - List all currently connected Libp2p Peer IDs
- \`/me\` - View your local details (Nickname, Peer ID, Multiaddresses)
- \`/exit\` - Shut down the node and close network tunnels gracefully
`;
